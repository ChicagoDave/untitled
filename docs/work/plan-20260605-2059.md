...
- **ADR-0011** (`0011-editor-shell-separate-swiftpm-package.md`) — app-target structure: new workspace files (`WorkspaceModel.swift`, tests) belong in `app/Sources/Galley/` and `app/Tests/GalleyTests/` respectively; the `GalleyShell` library target is for headlessly-testable logic only. Code that holds `AppKit`/`SwiftUI` types (command groups, menu building) lives in the `Galley` executable target.
