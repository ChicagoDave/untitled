...
**Added**: 2026-06-05
**Relationship to Build Step 2**: Phases 5 and 6 above (reference system, chapter templates) remain PENDING and are unaffected. This track is additive — it replaces the single-model app shell with a workspace container, but does not touch `GalleyCore`, `GalleyShell`, or the editing/reveal surfaces established in Phases 1–4.
**Bounded contexts touched**: N/A — app-layer state and menu infrastructure only.
**Key domain language**: WorkspaceModel (buffer container), buffer slot, current buffer, unsaved buffer
...
**Workspace container abstraction (ADR-0016 candidate)**: `WorkspaceModel` is an `@Observable` class that owns `[DocumentModel]` + `currentIndex: Int`. It is the single source of truth for which buffers are open and which is current. `Galley.swift` owns exactly one `WorkspaceModel`; `ContentView` receives it and derives `currentModel`. This is a store — Boundary Statement required before editing.
...
- **Files touched**:
  - NEW `app/Sources/Galley/WorkspaceModel.swift` — the store
  - `app/Sources/Galley/Galley.swift` — replace `@State var model` with `@State var workspace`; rewire `.commands`
  - `app/Sources/Galley/ContentView.swift` — accept `WorkspaceModel`, derive `currentModel`
  - `app/Sources/Galley/DocumentModel.swift` — add an `hasContent: Bool` computed property (true if any run has non-empty text); make `open()` return the URL it chose (so `WorkspaceModel` can call `load(from:)` on a new blank rather than mutating the existing model in-place)
  - `app/Tests/GalleyTests/WorkspaceModelTests.swift` — NEW
- **Behavior Statements to write** (rule 12, before tests):
  - `WorkspaceModel.new()` — DOES: appends a blank `DocumentModel` and sets `currentIndex` to its position. REJECTS WHEN: never rejects.
  - `WorkspaceModel.open(url:)` — DOES: appends a loaded `DocumentModel` from `url` and switches to it. REJECTS WHEN: `DocumentBundle.read` throws; in that case no buffer is appended and the workspace is unchanged.
  - `WorkspaceModel.switchTo(index:)` — DOES: auto-saves the current buffer if `fileURL != nil`, then sets `currentIndex`. REJECTS WHEN: index out of range (no-op, does not crash).
- **ADR-worthy**: `WorkspaceModel` container abstraction (ADR-0016); unsaved-New buffer policy (ADR-0015).
- **Boundary Statement required**: Yes — `WorkspaceModel` lives under the store/state pattern. Produce the statement before editing.
...
- **Files touched**:
  - `app/Sources/Galley/WorkspaceModel.swift` — add `close(index:)` with empty-blank-discard and last-buffer replacement logic
  ...
  - `app/Tests/GalleyTests/WorkspaceModelTests.swift` — extend with close/last-buffer tests
- **Behavior Statements to write**:
  - `WorkspaceModel.close(index:)` — DOES: removes the buffer at `index`; if it was the last buffer, replaces with a fresh blank; switches `currentIndex` to the nearest neighbor (prefer `index - 1`, else `index`, clamped). REJECTS WHEN: if the closing buffer has content and `fileURL == nil`, emits a `.unsavedContent(DocumentModel)` result that the caller uses to present a Save/Discard sheet before proceeding.
